## Notes for Infinispan test suite

### Howto
Setup the phyton environment.
...
Start Infinispan and run the following.

Restart the server after each command, cleanup doesn't work

```bash
python3 -m RLTest -v --existing-env-addr 127.0.0.1:6379 --no-progress -t test_multi:testSetAndGetCommands -t test:testSetAddNewImmediateChild
```

```bash
python3 -m RLTest -v --existing-env-addr 127.0.0.1:6379 --no-progress -t test:testSetBehaviorModifyingSubcommands -t test:testSetGetWholeBasicDocumentShouldBeEqual -t test:testSetGetWithSpecialKey
# Fails: appending a value at the end of an array succeded instead of failing
```

```bash
python3 -m RLTest -v --existing-env-addr 127.0.0.1:6379 --no-progress -t test:testSetInvalidPathShouldFail -t test:testSetReplaceRootShouldSucceed
```

```bash
python3 -m RLTest -v --existing-env-addr 127.0.0.1:6379 --no-progress -t test:testSetRootWithInvalidJSONValuesShouldFail -t test:testSetRootWithJSONValuesShouldSucceed -t test:testSetWithBracketNotation
```

```bash
python3 -m RLTest -v --existing-env-addr 127.0.0.1:6379 --no-progress -t test:testSetWithPathErrors
